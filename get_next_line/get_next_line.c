...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/12/19 05:09:02 by mboubaza          #+#    #+#             */
/*   Updated: 2025/12/21 16:06:02 by mboubaza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static char	*new_line(char *tmp, int fd)
{
	...
	while (readed > 0 && !ft_strchr(tmp, '\n'))
	{
		...
		if (readed == -1)
			return (free_buf(buffer), free_buf(tmp));
		buffer[readed] = '\0';
		tmptmp = tmp;
...
char	*get_next_line(int fd)
{
	...
	if (!tmp)
		return (NULL);
	str = the_line(tmp, &start);
	tmptmp = tmp;
	tmp = the_rest(tmptmp, start);
	free_buf(tmptmp);
	return (str);
}
